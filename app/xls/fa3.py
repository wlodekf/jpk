...
def faktura(jpk, workbook, dom):
    """
    Utworzenie arkusza Excela z JPK_FA wersja 3.
    """
    ...
    for element in dom.iter("{*}Faktura"):
        ...
        worksheet.write(row, 7, t(element.find('{*}KodWaluty')))
        
        worksheet.write_number(row, 8, kwota(element.find('{*}P_13_1')))  
        worksheet.write_number(row, 9, kwota(element.find('{*}P_14_1')))  
        worksheet.write_number(row, 10, kwota(element.find('{*}P_13_2')))  
        worksheet.write_number(row, 11, kwota(element.find('{*}P_14_2')))                          
        worksheet.write_number(row, 12, kwota(element.find('{*}P_13_3')))  
        worksheet.write_number(row, 13, kwota(element.find('{*}P_14_3')))  
        worksheet.write_number(row, 14, kwota(element.find('{*}P_13_4')))  
        worksheet.write_number(row, 15, kwota(element.find('{*}P_14_4'))) 
        worksheet.write_number(row, 16, kwota(element.find('{*}P_13_5')))  
        ...
        worksheet.write(row, 23, element.find('{*}P_18A').text) # nowy     
        worksheet.write(row, 24, element.find('{*}P_19').text)
        worksheet.write(row, 25, t(element.find('{*}P_19A')))
        worksheet.write(row, 26, element.find('{*}P_20').text)    
        worksheet.write(row, 27, element.find('{*}P_21').text)
        worksheet.write(row, 28, element.find('{*}P_22').text)
        worksheet.write(row, 29, element.find('{*}P_23').text)    
        worksheet.write(row, 30, element.find('{*}P_106E_2').text)

        worksheet.write(row, 31, element.find('{*}RodzajFaktury').text)
        worksheet.write(row, 32, t(element.find('{*}PrzyczynaKorekty')))
        worksheet.write(row, 33, t(element.find('{*}NrFaKorygowanej')))
        worksheet.write(row, 34, t(element.find('{*}OkresFaKorygowanej')))

        worksheet.write(row, 35, t(element.find('{*}NrFaZaliczkowej')))
                
        worksheet.write_string(row, 36, element.find('{*}P_3C').text)
        worksheet.write_string(row, 37, element.find('{*}P_3D').text)
        worksheet.write_string(row, 38, element.find('{*}P_4A').text)
        worksheet.write_string(row, 39, element.find('{*}P_4B').text)        
    ...
    worksheet.print_area(0,0,row,20)
