...
def raport_kontrolny(imp, dane):
    ...
    with closing(xlsxwriter.Workbook(output)) as workbook:
        # Dodanie arkusza
        zakupy_xls(imp, dane, workbook)
    
    return output.getvalue()


def zakupy_xls(imp, dane, workbook):
    ...
    formats= workbook.add_format({'bg_color': '#E6E6FF', 'color': '#000000'})
    ...
    for k, zak in dane:
        ...
        row_h= 15
        if len(k['kon'].get('nazwa1', '')) > 30 or len(k['analityka'])>20 or len(k['projekt'])>20:
            row_h= 30
        ...
        text(2, k['numerDokumentuZewnetrznego'])
        text(3, k['dataDokumentuZewnetrznego'][:10])
        ...
        text(8, k['rodzajDokumentuKosztowegoNazwaSkrocona'])
        text(9, k['platnosc'])

        kwota(10, k['wartoscNetto'], formatk)
        kwota(11, k['wartoscVAT'], formatk)
        kwota(12, k['wartoscBrutto'], formatk)
        
        text(13, k['analityka'])
        text(14, k['projekt'])
        text(15, k['numerDokumentu'])
        text(16, k['opis'])
        
        text(17, k['status'], formats)
        
        text(18, zak.rodzaj if zak else '')
        kwota(19, zak.lp if zak else None, formatl)
